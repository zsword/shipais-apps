...
func (ec elasticsClient) Count(index string, params map[string]interface{}) (count int64, err error) {
	es := ec.db
	count = -1
	buf, err := json.Marshal(params)
	if err != nil {
		logs.Error("Marshal query body: %v", err)
	}
	res, err := es.Count(es.Count.WithIndex(index), es.Count.WithBody(strings.NewReader(string(buf))))
	if err != nil {
		logs.Error("Count Index: %s: %v", index, err)
		return
	}
	data := countResponse{}
	...
	res.Body.Close()
	if err != nil {
		logs.Error("Parse count result: %v", err)
		return
	}
	count = data.Count
	return
